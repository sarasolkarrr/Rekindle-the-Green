...
$userInitials = $isLoggedIn ? strtoupper(substr($userName, 0, 1)) : '';
?>

<script>
  <?php if ($isLoggedIn): ?>
    localStorage.setItem('rtg_user_name', '<?= addslashes($userName) ?>');
    localStorage.setItem('rtg_user_id', '<?= $_SESSION['user_id'] ?>');
  <?php else: ?>
...
<nav>
  ...
  <div class="nav-actions">
    <?php if (!isset($_SERVER['PHP_SELF']) || strpos($_SERVER['PHP_SELF'], 'index') === false): ?>
      <a href="index.html" class="btn-nav btn-home">🏠 Home</a>
    <?php endif; ?>

    <?php if ($isLoggedIn): ?>
      <a href="profile.php" class="nav-user">
        <span><?= htmlspecialchars(substr($userName, 0, 12)) ?></span>
